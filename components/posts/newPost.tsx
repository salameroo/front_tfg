...
export default function CreatePostPage() {
    ...
    const handleChange = async (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        if (name === 'images' && files) {
            ...
            const newCompressedImages: File[] = await Promise.all(newImages.map(async (file) => {
                const options = {
                    maxSizeMB: 1, // Ajusta el tamaño máximo en MB según tus necesidades
                ...
                try {
                    ...
                    return file; // Si falla la compresión, usa la imagen original
                }
            }));
            const newImagePreviews: string[] = newCompressedImages.map(file => URL.createObjectURL(file));
            setImages(prevImages => [...prevImages, ...newCompressedImages]);
